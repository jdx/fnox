...
impl Config {
    ...
    fn load_recursive(dir: &Path, _from_parent: bool, found_any: bool) -> Result<(Self, bool)> {
        ...
        // Load fnox.$FNOX_PROFILE.toml or .fnot.$FNOX_PROFILE.toml if it exists and merge it (takes precedence over fnox.toml/.fnot.toml)
        ...
        // Load fnox.local.toml or .fnot.local.toml if it exists and merge it (takes precedence over fnox.$FNOX_PROFILE.toml/.fnot.$FNOX_PROFILE.toml)
    ...
    pub fn get_default_provider(&self, profile: &str) -> Result<Option<String>> {
        ...
        if providers.is_empty() {
            return Err(FnoxError::Config(
                "No providers configured. Add at least one provider to fnox.toml or .fnox.toml"
                    .to_string(),
            ));
        }
    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if self.providers.is_empty() && self.profiles.is_empty() && !self.secrets.is_empty() {
            return Err(FnoxError::Config(
                "No providers configured. Add at least one provider to fnox.toml or .fnot.toml"
                    .to_string(),
            ));
        }
