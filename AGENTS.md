...
`fnox-core` exposes per-provider Cargo features so downstream
embedders can drop providers they don't ship and shrink their binary.
Defaults enable everything, so end-users and packagers see no change.

**Adding a new gated provider requires changes in three places that
must stay in sync.** Skipping any of them produces either a build
that always includes the provider (defeating the feature) or a test
that drifts and fails. The drift test in
`src/commands/provider/mod.rs::tests` panics loudly if you miss the
last step, but it's worth doing all three together:

1. **`crates/fnox-core/providers/<name>.toml`** — add
   `cargo_feature = "<name>"`. The build script reads this and
   wraps every generated reference to the provider (`ProviderConfig`
   variants, match arms, `use` statements, wizard entries) in
   `#[cfg(feature = "<name>")]`.
2. **`crates/fnox-core/Cargo.toml`** — mark the provider's dep(s)
   `optional = true` and add the matching feature:
   `<name> = ["dep:<crate>"]`. Add the feature to `all-providers`.
3. **Binary side** (when the provider is referenced from
   `src/commands/...`):
   - Mirror the feature in the top-level `Cargo.toml`:
     `<name> = ["fnox-core/<name>"]`, plus `all-providers` includes
     it.
   - Cfg-gate every direct mention with `#[cfg(feature = "<name>")]`
     (clap enum variants, match arms, helper functions).
   - Add a `cfg!(feature = "<name>")` arm to
     `provider_feature_enabled` in `src/commands/provider/mod.rs::tests`
     so the drift test correctly excludes the provider when the
     feature is off. `cfg!()` requires a string literal, which is
     why this can't be done generically the way the build script
     handles step 1.

Also extend `.github/workflows/ci.yml`'s `feature-combos` matrix
with a relevant slug if the new provider deserves dedicated coverage
(`--no-default-features --features <name>` etc.).
